 ...
 * Returns, Shipping, About, FAQ, Contact, Genuine-vs-Compatible.
 *
 ...
 *
 * WHAT THIS FILE DELIBERATELY DOES NOT DO — read before adding anything
 * ---------------------------------------------------------------------
 * It performs **no network I/O of any kind**, and it never injects copy
 * from a remote source into a legal page. That is a compliance property,
 * not an oversight, and it is asserted by
 * `tests/legal-cms-retired-jul2026.test.js` — including by grepping this
 * very file for the identifiers of the mechanism described below, which
 * is why they are not spelled out here.
 *
 * Until 2026-07-14 this file read admin-authored rows from a remote
 * content table and wrote them into `.policy-section` via innerHTML — a
 * legal-content CMS. It never actually worked (ERR-065: `js/config.js`
 * declares `const Config`, which is not a property of `window`, so the
 * read always short-circuited), and the owner chose to retire the CMS
 * rather than repair it.
 *
 * The reason is worth keeping: the backend's bot prerender does not read
 * that table. An override that rendered in the browser but not in the
 * prerendered HTML would make bot copy differ from human copy on /terms
 * and /about — which is **cloaking**, the exact charge under appeal with
 * Google Ads. One vetted source per legal page (the HTML, plus
 * legal-config.js for the facts) is the whole point.
 *
 * So: to change legal copy, edit the page's HTML or legal-config.js.
 * Never give this file a way to pull copy off the network.
 * See ERR-065 / ERR-069 in errors.md.
 */
(function () {
    'use strict';
    ...
    // Render synchronously off the static HTML. There is nothing to wait
    // for — see the CMS note in the file header.
    function init() {
        renderStatic();
    }
