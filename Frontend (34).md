...
> **Updated:** 2026-03-04 (full audit: fixed ~80 stale line numbers, added feed-files & admin export customers, corrected admin analytics line refs) | **Generated:** 2026-02-18 | **Source:** Zero-assumption audit of current backend source code
...
**Entry point:** `src/server.js` — Express app exported at line 375.

**Middleware order** (`src/server.js:103-276`):
```
trust proxy (L103) → helmet (L109) → HTTPS redirect prod-only (L124)
→ CORS (L143) → requestId (L170)
→ global rate limit 100/min (L173-177) → request timeouts 15s/30s/60s (L181-183)
→ compression (L186) → raw body for webhooks BEFORE json (L190)
→ express.json 1mb (L193) → urlencoded 1mb (L194)
→ cookieParser (L197) → morgan (L200, combined in prod / dev in dev)
→ health check (L207) → readiness check (L228) → root info (L258)
→ SEO routes (L263) → API routes (L267)
→ Sentry error handler (L270, conditional) → 404 handler (L275) → error handler (L276)
```
...
- HTTPS enforced via `x-forwarded-proto` redirect (`src/server.js:124-132`)
...
| Path | File | Purpose |
|------|------|---------|
| `GET /health` | `src/server.js:207` | Health check |
| `GET /ready` | `src/server.js:228` | Readiness check (DB + Stripe) |
| `GET /` | `src/server.js:258` | API info |
...
Raw body middleware configured in `src/server.js:190` BEFORE `express.json()`.
...
`admin.js:147` | **Auth:** `requireAdmin`
Returns admin role info (`is_admin`, `role`, `roles`, `email`). Used by frontend to confirm admin access on load.

#### GET /api/admin/orders
`admin.js:167` | **Auth:** `requireAdmin` + `adminUserLimiter` (30/min per user) + `requireRole('super_admin', 'order_manager')`
...
`admin.js:326` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params)
...
`admin.js:457` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')`
...
`admin.js:2388` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params)
...
`admin.js:2426` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params) + `createOrderEventSchema` (body)
...
`admin.js:2470` | **Validation:** `adminRefundQuerySchema` (query)
...
`admin.js:2547` | **Validation:** `createRefundSchema` (body)
...
`admin.js:2606` | **Validation:** `refundIdSchema` (params) + `updateRefundSchema` (body)
...
`admin.js:2657` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager', 'stock_manager')` | **Validation:** `adminExportTypeSchema` (params) + `adminExportQuerySchema` (query)
...
`admin.js:722` | **Auth:** `requireAdmin`
...
`admin.js:835` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:916` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:985` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1052` | **Auth:** `requireAdmin`
...
`admin.js:1151` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1278` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1387` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1468` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:2035` | Query: `page`, `limit`, `status`. **Validation:** `adminBusinessApplicationQuerySchema`.

#### GET /api/admin/business-applications/:applicationId
`admin.js:2108` | **Validation:** `adminBusinessApplicationIdSchema` (params).

#### PUT /api/admin/business-applications/:applicationId
`admin.js:2182` | Body: `{ "status": "approved"|"rejected", "notes" }`. **Validation:** `updateBusinessApplicationSchema`.
...
`admin.js:2283` | Returns application statistics (counts by status, recent applications).
...
`admin.js:1917` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminCustomerQuerySchema` (query)
...
|--------|------|------|---------|
| POST | `/api/admin/cron/cleanup-emails` | L1580 | Clean stuck email queue entries. Body: `{ "max_age_hours": int }` (1–168, default 24). Validated by `cronCleanupEmailsSchema`. |
| POST | `/api/admin/cron/process-emails` | L1597 | Process pending email queue |
| POST | `/api/admin/cron/cleanup-data` | L1613 | Clean expired sessions, old carts |
| POST | `/api/admin/cron/reconcile-payments` | L1679 | Reconcile Stripe payments with orders |
| POST | `/api/admin/cron/retry-post-payment` | L1697 | Retry failed post-payment tasks |
| POST | `/api/admin/cron/cart-recovery` | L2356 | Send abandoned cart recovery emails |
| POST | `/api/admin/cron/cleanup-guest-carts` | L2372 | Remove expired guest carts |
| POST | `/api/admin/cron/daily-product-import` | L3260 | Run full import pipeline (genuine → compatible → ribbons). Returns per-script results. 200 on full success, 207 on partial failure. |
| POST | `/api/admin/import/genuine` | L3043 | Trigger genuine products import (DSNZ.xlsx). 10-min timeout. 409 if already running. **400 if body is non-empty.** |
| POST | `/api/admin/import/compatible` | L3117 | Trigger compatible products import (Augmento.xlsx). 10-min timeout. 409 if already running. **400 if body is non-empty.** |
| POST | `/api/admin/import/ribbons` | L3190 | Trigger ODS ribbon import (RIBBONS Streamline.ods). 10-min timeout. 409 if already running. |
| GET | `/api/admin/recovery/health-check` | L1633 | System health check |
| POST | `/api/admin/recovery/fix-missing-invoices` | L1646 | Fix orders missing invoice numbers |
| POST | `/api/admin/recovery/cancel-stale-orders` | L1661 | Cancel unpaid orders past threshold (1h default, runs every 30min via cron) |
| GET | `/api/admin/recovery/data-integrity-audit` | L1713 | Run full data integrity audit |
...
|--------|------|------|------|---------|
| POST | `/api/admin/feed-files/:feedType` | L2977 | `verifyCronAuth` | Upload feed file to Supabase Storage. Multipart form with `file` field. Valid `feedType`: `genuine`, `compatible`, `ribbons`. Magic byte validated (.xlsx/.ods). Returns `{ message, filename, size }`. |
| GET | `/api/admin/feed-files` | L3016 | `verifyCronAuth` | List all feed files in storage with metadata. Returns `{ files: [...] }`. |
...
**GA4 (in `admin.js`):**
- `GET /api/admin/analytics/ga4-summary` — `admin.js:2331` | Returns GA4 event summary (admin-only, no validation schema)
...
|--------|------|-----------|------|---------|
| GET | `/health` | `server.js:207` | None | `{ "ok": true, "data": { "message": "Server is running", "timestamp": "ISO" } }` |
| GET | `/ready` | `server.js:228` | None | `{ "ok": true/false, "data": { "ready": bool, "checks": { "db", "stripe" } } }` |
| GET | `/` | `server.js:258` | None | `{ "ok": true, "data": { "message": "Ink Cartridge E-Commerce API", "version": "v1" } }` |
...
|-------|--------|-----|-----------|
| Global `/api/` | 1 min | 100 | `server.js:173-177` |
...
| **Admin (per user)** | **1 min** | **30** | `admin.js:135-142` |
...
### Body Size Limits
`express.json({ limit: '1mb' })` and `express.urlencoded({ limit: '1mb' })` (`server.js:193-194`).
...
### Trust Proxy
`app.set('trust proxy', 1)` — trusts single proxy layer for X-Forwarded-* headers (`server.js:103`).
...
### Webhook Raw Body
- `express.raw({ type: 'application/json' })` for `/api/webhooks/payment` MUST be registered before `express.json()` in `server.js:190`
...
- GA4 summary: `admin.js:2297` → `admin.js:2331`
