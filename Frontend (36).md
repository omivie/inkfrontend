...
> **Updated:** 2026-03-08 (line-number audit: fixed server.js, index.js, products.js, admin.js, adminShipping.js, adminSupplierOffers.js refs) | **Generated:** 2026-02-18 | **Source:** Zero-assumption audit of current backend source code
...
| sharp | ^0.34.5 | Image processing (pack composites) |
...
**Entry point:** `src/server.js` — Express app exported at line 376.

**Middleware order** (`src/server.js:104-277`):
```
trust proxy (L104) → helmet (L110) → HTTPS redirect prod-only (L125)
→ CORS (L135) → requestId (L171)
→ global rate limit 100/min (L174-178) → request timeouts 15s/30s/60s (L181-183)
→ compression (L187) → raw body for webhooks BEFORE json (L191)
→ express.json 1mb (L194) → urlencoded 1mb (L195)
→ cookieParser (L198) → morgan (L201, combined in prod / dev in dev)
→ health check (L208) → readiness check (L229) → root info (L259)
→ SEO routes (L263) → API routes (L268)
→ Sentry error handler (L272, conditional) → 404 handler (L276) → error handler (L277)
```
...
- Port: `process.env.PORT || 3000` (`src/server.js:106`)

**Production:**
- API base derived from `ALLOWED_ORIGINS` env var
- HTTPS enforced via `x-forwarded-proto` redirect (`src/server.js:125-133`)
...
**CORS Configuration** (`src/server.js:135-167`):
- Origins: `process.env.ALLOWED_ORIGINS` (comma-separated) or `['http://localhost:3000']` default
- Localhost pattern `/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/` allowed in development only (`NODE_ENV === 'development'`, L151)
- No-origin requests allowed (mobile apps, Postman, webhooks) (L146)
- `credentials: true` — cookies sent cross-origin (L163)
- Methods: `GET, POST, PUT, DELETE, OPTIONS` (L164)
- Allowed headers: `Content-Type, Authorization, X-Requested-With, X-Request-Id` (L165)
- Preflight cache: 86400s / 24 hours (L166)
- Blocked origins logged: `console.warn('CORS blocked origin: ...')` (L159)

**Non-API routes** (mounted at root, not `/api`):
| Path | File | Purpose |
|------|------|---------|
| `GET /health` | `src/server.js:208` | Health check |
| `GET /ready` | `src/server.js:229` | Readiness check (DB + Stripe) |
| `GET /` | `src/server.js:259` | API info |
...
### Route Registration (`src/routes/index.js:272-293`)

| Mount | Router File | Prefix |
|-------|------------|--------|
| L272 | `products.js` | `/api` |
| L273 | `orders.js` | `/api` |
| L274 | `cart.js` | `/api` |
| L275 | `webhooks.js` | `/api` |
| L276 | `user.js` | `/api` |
| L277 | `admin.js` | `/api` |
| L278 | `search.js` | `/api` |
| L279 | `analytics.js` | `/api/analytics` |
| L280 | `auth.js` | `/api` |
| L281 | `business.js` | `/api` |
| L282 | `shipping.js` | `/api` |
| L283 | `settings.js` | `/api` |
| L284 | `cartAnalytics.js` | `/api` |
| L285 | `newsletter.js` | `/api` |
| L286 | `contact.js` | `/api` |
| L287 | `reviews.js` | `/api` |
| L288 | `account.js` | `/api` |
| L289 | `ribbons.js` | `/api` |
| L290 | `adminRibbons.js` | `/api` |
| L291 | `adminShipping.js` | `/api` |
| L292 | `adminSupplierOffers.js` | `/api` |
| L293 | `adminAnalytics.js` | `/api/admin/analytics` |
...
**Line:** 208 | **Auth:** None | **Validation:** `productCountsQuerySchema` (query)

Returns category counts for a brand — use on the brand landing page instead of fetching all products to count categories.

Query params:
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| brand | string | — | **Required.** Brand slug (e.g. `brother`) |
| source | string | — | `genuine` or `compatible` (optional filter) |

Categories:
- `ink` — `ink_cartridge`, `ink_bottle`
- `toner` — `toner_cartridge`
- `drums` — `drum_unit`, `belt_unit`, `fuser_kit`, `waste_toner`

Zero-count categories are omitted from the response.

Response:
```json
{
  "ok": true,
  "data": { "ink": 301, "toner": 254, "drums": 41 }
}
```

#### GET /api/products/series
**Line:** 322 | **Auth:** None | **Validation:** `productSeriesQuerySchema` (query)

Returns series codes with product counts for a brand — use on the product code grid (e.g. Brother ink → LC23, LC233, TN257, etc.) instead of fetching all products to extract series.

Query params:
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| brand | string | — | **Required.** Brand slug |
| category | string | — | `ink`, `toner`, `drums`, or `consumable` (optional) |
| source | string | — | `genuine` or `compatible` (optional filter) |

Series codes are extracted from product names (stripping brand, color suffixes, etc.) and naturally sorted. A product with multiple codes (e.g. "LC77 LC73 LC40 Ink") is counted under each code.
...
**Line:** 390 | **Auth:** None | **Validation:** `productSkuSchema` (params)

Response includes: product detail, `compatible_printers` (for non-ribbon consumables), `compatible_cartridges` (for printers), `in_stock`, `is_low_stock`. `cost_price` always returned as `null` (L488). Brand includes `logo_path` (full URL, use directly). For ribbon detail with `compatible_devices`, use the dedicated `GET /api/ribbons/:sku` endpoint instead.

#### GET /api/products/printer/:printerSlug
**Line:** 502 | **Auth:** None | **Validation:** `printerSlugSchema` (params)

Returns printer info + `compatible_products` array with `in_stock` flag.

#### GET /api/printers/search
**Line:** 573 | **Auth:** None | **Validation:** `printerSearchSchema` (query)

Query: `q` (required), `brand` (optional slug). Results capped at 10 (hardcoded at `products.js:592`).
Returns printers with `compatible_products_count`.

#### GET /api/brands
**Line:** 645 | **Auth:** None | **Validation:** None
...
**Line:** 673 | **Auth:** None | **Validation:** `printerIdSchema` (params)

Returns `{ printer, compatible_products, total_compatible }`.

#### GET /api/products/printer/:printerSlug/color-packs
**Line:** 762 | **Auth:** None | **Validation:** `printerSlugSchema` (params)
...
**Line:** 812 | **Auth:** None | **Validation:** None
...
**Line:** 572 | **Auth:** `optionalAuth` | **Validation:** `smartSearchSchema` (query, defined locally in `search.js:47-50`)
...
#### POST /api/orders
**Line:** 126 | **Auth:** `requireAuth` + `requireVerifiedEmail` | **Rate Limit:** `orderLimiter` 10/15min per user | **Validation:** `createOrderSchema` (body)
...
**Line:** 824 | **Auth:** `requireAuth` | **Validation:** None
...
#### GET /api/orders
**Line:** 856 | **Auth:** `requireAuth` | **Validation:** `orderQuerySchema` (query)
...
**Line:** 983 | **Auth:** `requireAuth` | **Validation:** `orderNumberParamSchema` (params — must match `ORD-{base36}-{hex4}` format, max 30 chars)
...
**Rate limit:** 3/hour (`contactLimiter`, L12-18)
**Bot protection:** Cloudflare Turnstile (when `TURNSTILE_SECRET_KEY` is configured)
...
Raw body middleware configured in `src/server.js:191` BEFORE `express.json()`.
...
`admin.js:149` | **Auth:** `requireAdmin`
Returns admin role info (`is_admin`, `role`, `roles`, `email`). Used by frontend to confirm admin access on load.

#### GET /api/admin/orders
`admin.js:169` | **Auth:** `requireAdmin` + `adminUserLimiter` (30/min per user) + `requireRole('super_admin', 'order_manager')`
...
`admin.js:328` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params)
...
**Supplier fulfillment** (super_admin only): the response includes `supplier_fulfillment` with the cheapest supplier chosen at checkout:
- `selected_supplier` — supplier name (e.g. "Augmento", "Supplier2026")
- `total_supplier_cost` — total cost from that supplier for all compatible items
- `line_details` — array of `{ canonical_sku, supplier_sku, quantity, unit_cost, line_cost }`
- `null` if the order has no compatible items or no fulfillment record exists

#### PUT /api/admin/orders/:orderId
`admin.js:467` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')`
...
`admin.js:2397` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params)
...
`admin.js:2435` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminOrderIdSchema` (params) + `createOrderEventSchema` (body)
...
`admin.js:2483` | **Validation:** `adminRefundQuerySchema` (query)
...
`admin.js:2560` | **Validation:** `createRefundSchema` (body)
...
`admin.js:2619` | **Validation:** `refundIdSchema` (params) + `updateRefundSchema` (body)
...
`admin.js:2670` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager', 'stock_manager')` | **Validation:** `adminExportTypeSchema` (params) + `adminExportQuerySchema` (query)
Path param: `orders`, `refunds`, or `customers`. Query: `from` (YYYY-MM-DD), `to` (YYYY-MM-DD), `statuses` (comma-separated).
...
**Customers CSV columns:** `Name, Email, Phone, Account Type, Created At, Order Count, Total Spent`
...
`admin.js:732` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:844` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:925` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:994` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1061` | **Auth:** `requireAdmin`
...
`admin.js:1160` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1287` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1396` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:1477` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'stock_manager')`
...
`admin.js:2044` | Query: `page`, `limit`, `status`. **Validation:** `adminBusinessApplicationQuerySchema`.

#### GET /api/admin/business-applications/:applicationId
`admin.js:2117` | **Validation:** `adminBusinessApplicationIdSchema` (params).

#### PUT /api/admin/business-applications/:applicationId
`admin.js:2191` | Body: `{ "status": "approved"|"rejected", "notes" }`. **Validation:** `updateBusinessApplicationSchema`.
...
`admin.js:2292` | Returns application statistics (counts by status, recent applications).
...
`admin.js:1926` | **Auth:** `requireAdmin` + `requireRole('super_admin', 'order_manager')` | **Validation:** `adminCustomerQuerySchema` (query)
...
`adminShipping.js:119` | **Validation:** `adminShippingRateQuerySchema` (query)
...
### 4.19f Admin Supplier Offers

**File:** `src/routes/adminSupplierOffers.js`
**Auth:** `requireAdmin` + `requireRole('super_admin')`

#### GET /api/admin/supplier-offers/compare
`adminSupplierOffers.js:13` | **Validation:** `supplierOffersCompareSchema` (query)

Compares pricing across compatible product suppliers. Returns products that have offers from 2+ suppliers, showing which supplier is cheapest and the cost difference.

**Query params:** `page` (default 1), `limit` (default 50, max 200), `brand` (filter by brand name), `product_type` (filter by type).
...
```json
{
  ...
  "data": {
    "comparisons": [
      {
        "product_id": "uuid",
        "sku": "INK-BR-LC432XL-BK",
        "name": "Brother LC-432XL Black",
        "brand": "Brother",
        "product_type": "cartridge",
        "canonical_sku": "BROTHER:LC432XL:cartridge:black",
        "cheapest_supplier": "Supplier2026",
        "cost_difference": 1.25,
        "suppliers": [
          { "supplier_name": "Supplier2026", "supplier_sku": "S2026-LC432", "cost_nzd": 5.50, "last_seen_at": "..." },
          { "supplier_name": "Augmento", "supplier_sku": "AUG-LC432XL-BK", "cost_nzd": 6.75, "last_seen_at": "..." }
        ]
      }
    ],
    "pagination": { "page": 1, "limit": 50, "total": 120, "total_pages": 3, "has_next": true, "has_prev": false }
  }
}
```

Sorted by `cost_difference` descending (biggest savings first).
...
|--------|------|------|---------|
| POST | `/api/admin/cron/cleanup-emails` | L1589 | Clean stuck email queue entries. Body: `{ "max_age_hours": int }` (1–168, default 24). Validated by `cronCleanupEmailsSchema`. |
| POST | `/api/admin/cron/process-emails` | L1606 | Process pending email queue |
| POST | `/api/admin/cron/cleanup-data` | L1622 | Clean expired sessions, old carts |
| POST | `/api/admin/cron/reconcile-payments` | L1688 | Reconcile Stripe payments with orders |
| POST | `/api/admin/cron/retry-post-payment` | L1706 | Retry failed post-payment tasks |
| POST | `/api/admin/cron/cart-recovery` | L2365 | Send abandoned cart recovery emails |
| POST | `/api/admin/cron/cleanup-guest-carts` | L2381 | Remove expired guest carts |
| POST | `/api/admin/cron/daily-product-import` | L3203 | Run full import pipeline (genuine → compatible). Returns per-script results. 200 on full success, 207 on partial failure. |
| POST | `/api/admin/import/genuine` | L3056 | Trigger genuine products import (DSNZ.xlsx). 10-min timeout. 409 if already running. **400 if body is non-empty.** |
| POST | `/api/admin/import/compatible` | L3130 | Trigger compatible products import (Augmento.xlsx). 10-min timeout. 409 if already running. **400 if body is non-empty.** |
| GET | `/api/admin/recovery/health-check` | L1642 | System health check |
| POST | `/api/admin/recovery/fix-missing-invoices` | L1655 | Fix orders missing invoice numbers |
| POST | `/api/admin/recovery/cancel-stale-orders` | L1670 | Cancel unpaid orders past threshold (1h default, runs every 30min via cron) |
| GET | `/api/admin/recovery/data-integrity-audit` | L1722 | Run full data integrity audit |

#### Feed File Management (Backend-Only)

Upload and list feed spreadsheet files in Supabase Storage. Used by cron/admin import triggers.

| Method | Path | Line | Auth | Purpose |
|--------|------|------|------|---------|
| POST | `/api/admin/feed-files/:feedType` | L2990 | `verifyCronAuth` | Upload feed file to Supabase Storage. Multipart form with `file` field. Valid `feedType`: `genuine`, `compatible`, `ribbons`. Magic byte validated (.xlsx/.ods). Returns `{ message, filename, size }`. |
| GET | `/api/admin/feed-files` | L3029 | `verifyCronAuth` | List all feed files in storage with metadata. Returns `{ files: [...] }`. |
...
**Top Products:**
- `GET /api/admin/analytics/top-products` — L2108 | **Validation:** `analyticsTopProductsSchema` (query). Query: `metric` (`revenue`/`units`/`profit`/`margin_pct`), `productType`, `compatibilityType`, `days` (default 30), `limit` (default 10). Returns `{ metric, productType, compatibilityType, days, products: [...] }`. Mirrors `/api/analytics/top-products`.
...
- `GET /api/admin/analytics/overview` — L2007 | Query: `days` or `timeRange` (default 7). Returns `{ grossProfit, netProfit, refundRate, avgFulfilmentTime, revenueSparkline[], ordersSparkline[], revenueTrend: { direction, change }, ordersTrend: { direction, change } }`
...
- `GET /api/admin/analytics/cac` — L788 | Query: `months`, `channel`
- `GET /api/admin/analytics/ltv-cac-ratio` — L840
- `GET /api/admin/analytics/cohorts` — L889 | Query: `months`, `metric`
- `GET /api/admin/analytics/churn` — L963 | Query: `include_at_risk`
- `GET /api/admin/analytics/customer-health` — L1021 | Query: `status`, `sort_by`, `limit`
- `GET /api/admin/analytics/nps` — L1115
- `POST /api/admin/analytics/feedback` — L1155 | **Validation:** `analyticsCreateFeedbackSchema` (body). Body: `{ user_id, order_id, feedback_type, nps_score (0-10), rating (1-5), comment, tags[] }`
- `GET /api/admin/analytics/repeat-purchase` — L1193

**Marketing (5 endpoints):**
- `GET /api/admin/analytics/campaigns` — L1240 | Query: `status`, `channel`
- `POST /api/admin/analytics/campaigns` — L1301 | **Validation:** `analyticsCreateCampaignSchema` (body). Body: `{ name, channel, budget, start_date, end_date, target_audience, goals }`
- `POST /api/admin/analytics/marketing-spend` — L1332 | **Validation:** `analyticsMarketingSpendSchema` (body). Body: `{ campaign_id (UUID, optional), channel, amount, date, impressions, clicks, conversions }`
- `GET /api/admin/analytics/channel-efficiency` — L1388
- `GET /api/admin/analytics/conversion-funnel` — L1437. Returns `data.funnel` (array of `{ stage, count, rate }`), `data.steps` (array of `{ label, value }` with human-readable labels: "Site Visits", "Add to Cart", "Checkout Started", "Order Completed"), `data.drop_off` (`{ cart_to_checkout, checkout_to_purchase }` percentages), and `data.overall_conversion_rate`.
...
- `GET /api/admin/analytics/inventory-turnover` — L1505 | Query: `sort_by`, `limit`
- `GET /api/admin/analytics/dead-stock` — L1573 | Query: `days_threshold`, `min_value`
- `GET /api/admin/analytics/stock-velocity` — L1633 | Query: `limit`, `sort_by`
- `GET /api/admin/analytics/inventory-cash-lockup` — L1700
- `GET /api/admin/analytics/product-performance` — L1755 | Query: `sort_by`, `limit`, `include_unprofitable`
- `GET /api/admin/analytics/page-revenue` — L1834

**Alerts (4 endpoints):**
- `GET /api/admin/analytics/alerts` — L1897 | Query: `severity`, `acknowledged` (true/false)
- `PUT /api/admin/analytics/alerts/:alertId/acknowledge` — L1930 | **Validation:** `analyticsAlertIdSchema` (params — UUID)
- `GET /api/admin/analytics/alert-thresholds` — L1956
- `PUT /api/admin/analytics/alert-thresholds/:thresholdId` — L1975 | **Validation:** `analyticsThresholdIdSchema` (params — UUID) + `analyticsUpdateThresholdSchema` (body). Body: `{ threshold_value, severity (low/medium/high/critical), is_enabled, cooldown_hours (0-720) }`

**GA4 (in `admin.js`):**
- `GET /api/admin/analytics/ga4-summary` — `admin.js:2340` | Returns GA4 event summary (admin-only, no validation schema)
...
|--------|------|-----------|------|---------|
| GET | `/health` | `server.js:208` | None | `{ "ok": true, "data": { "message": "Server is running", "timestamp": "ISO" } }` |
| GET | `/ready` | `server.js:229` | None | `{ "ok": true/false, "data": { "ready": bool, "checks": { "db", "stripe" } } }` |
| GET | `/` | `server.js:259` | None | `{ "ok": true, "data": { "message": "Ink Cartridge E-Commerce API", "version": "v1" } }` |
...
|-------|--------|-----|-----------|
| Global `/api/` | 1 min | 100 | `server.js:174-178` |
...
| **Admin (per user)** | **1 min** | **30** | `admin.js:137-144` |
...
| Contact | 1 hour | 3 | `contact.js:12-18` |
...
### HTTPS Enforcement
Production only, via `x-forwarded-proto` header from load balancer (`server.js:124-132`). 301 redirect. Health check paths (`/health`, `/api/health`) are exempt.

### Body Size Limits
`express.json({ limit: '1mb' })` and `express.urlencoded({ limit: '1mb' })` (`server.js:194-195`).
...
- `cost_price: null` in product detail response (`products.js:487`)
- `cost_price` stripped from search RPC results (`search.js:146`, `search.js:279`)

### Trust Proxy
`app.set('trust proxy', 1)` — trusts single proxy layer for X-Forwarded-* headers (`server.js:104`).
...
- **Images:** Composite images are auto-generated during import when all constituent cartridges have images. Individual cartridge images are arranged side-by-side in canonical color order (K→C→M→Y for KCMY, C→M→Y for CMY) and uploaded to `images/products/{packSKU}/composite.png` in Supabase storage. The pack's `image_url` is set to this path. Packs that already have an image are skipped. Failures are non-blocking — the pack is created without an image.
...
### Webhook Raw Body
- `express.raw({ type: 'application/json' })` for `/api/webhooks/payment` MUST be registered before `express.json()` in `server.js:191`
...
- **[MEDIUM] Printer search N+1 eliminated** (`products.js:614-628`). Compatible products count now uses `{ count: 'exact', head: true }` (Postgres-side count) instead of fetching all `product_compatibility` rows into memory.
...
### 2026-03-01 — ~~Add `compatibility` field to ribbon update endpoint~~ (NOT IMPLEMENTED)

> **Note:** This changelog entry was written but the code change was never applied. The `updateRibbonSchema` does NOT include a `compatibility` field, and `adminRibbons.js` has no `compatibility` mapping. Do NOT send `compatibility` in ribbon update requests — it will be silently ignored by Joi validation.
...
5. **[NEW] Admin import trigger endpoints** (`src/routes/admin.js`): `POST /api/admin/import/genuine` and `POST /api/admin/import/compatible` — triggers feed imports via admin panel or cron. Secured by `verifyCronAuth` (CRON_SECRET or super_admin). Returns parsed `summary` object (from `IMPORT_SUMMARY_JSON_V1`) on success instead of raw stdout. Error responses return generic message only (no stderr/internal details). 10-minute timeout. Concurrent runs blocked (409).
...
- `src/cron/jobs.js` — Added `dailyProductImport()` job: runs genuine → compatible sequentially with 20-min timeout per script. 30-minute lock TTL. (**Updated 2026-03-04:** Ribbon import removed from cron.)
- `src/cron/scheduler.js` — Daily product import scheduled at 2pm UTC (2am NZST). Sends email + Sentry alert on failure.
- `src/routes/admin.js` — Added `POST /api/admin/cron/daily-product-import` (trigger full import pipeline). Secured by `verifyCronAuth`. (**Updated 2026-03-04:** `POST /api/admin/import/ribbons` removed.)
...
**Import run order:**
The daily cron runs: `genuine.js` → `compatible.js`. Genuine must run first (compatible depends on it for compatibility links). Ribbon import (`import-ribbons.js`) is no longer part of the cron pipeline — ribbon data is managed separately. The daily cron at 2pm UTC (2am NZST) handles this automatically.

**New admin endpoints:**

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| POST | `/api/admin/cron/daily-product-import` | `verifyCronAuth` | Run full import pipeline (genuine → compatible). Returns per-script results. 207 on partial failure. |

**Cron schedule (`src/cron/scheduler.js`):**

| Schedule | Job | Description |
|----------|-----|-------------|
| `0 14 * * *` | `daily-product-import` | Runs genuine → compatible sequentially. 30-min distributed lock. Sends email + Sentry alert on any failure. |
...
### 2026-03-04 — Full Line Reference Audit + Remove Unimplemented Admin Analytics

**Doc-only changes (no code changes):**

Comprehensive audit of all `file:line` references against current backend source code. Code changes since last audit caused line numbers to drift.

**Fixes:**

*Section 1 (System Overview):*
- Entry point export line: 374 → 375
- Middleware pipeline: all `server.js` line references updated (+1 shift): trust proxy L102→L103, helmet L108→L109, HTTPS redirect L122→L124, CORS L142→L143, requestId L169→L170, rate limit L172-177→L173-177, timeouts L179-182→L181-183, compression L185→L186, raw body L189→L190, express.json L192→L193, urlencoded L193→L194, cookieParser L196→L197, morgan L199→L200, health L206→L207, readiness L227→L228, root L257→L258, API routes L266→L267, 404 L274→L275, error L275→L276

*Section 2 (Base URL Rules):*
- HTTPS enforcement: `server.js:122-131` → `server.js:124-132`
- CORS configuration: `server.js:133-166` → `server.js:135-167`
- Non-API routes table: health L206→L207, ready L227→L228, root L257→L258
- CORS sub-references: no-origin L145→L146, localhost L150→L151, credentials L162→L163, methods L163→L164, headers L164→L165, maxAge L165→L166, blocked L158→L159

*Section 4.2 (Search):*
- `GET /api/search/autocomplete`: L372 → L404 (off by 32)
- `GET /api/search/compatible-printers/:sku`: L448 → L451 (off by 3)
- `GET /api/search/smart`: L513 → L572 (off by 59)

*Section 4.3 (Orders):*
- `POST /api/orders`: L124 → L126
- `GET /api/orders/check-pending`: L820 → L824
- `GET /api/orders`: L852 → L856
- `GET /api/orders/:orderNumber`: L979 → L983

*Section 4.10 (Shipping):*
- `GET /api/shipping/rates`: L102 → L104

*Section 4.13 (Contact):*
- Rate limit: `contactLimiter, L11-17` → `L12-18`

*Section 4.17 (Admin Orders):*
- `GET /admin/verify`: L124 → L147 (off by 23)
- `GET /admin/orders`: L144 → L167 (off by 23)
- `GET /admin/orders/:orderId`: L303 → L326 (off by 23)
- `PUT /admin/orders/:orderId`: L434 → L457 (off by 23)
- `GET /admin/orders/:orderId/events`: L2354 → L2388 (off by 34)
- `POST /admin/orders/:orderId/events`: L2392 → L2426 (off by 34)

*Section 4.17b (Admin Refunds):*
- `GET /admin/refunds`: L2436 → L2470
- `POST /admin/refunds`: L2513 → L2547
- `PUT /admin/refunds/:refundId`: L2572 → L2606

*Section 4.17c (Admin Export):*
- `GET /admin/export/:type`: L2623 → L2657
- **Added `customers` export type** — was implemented in code but missing from documentation

*Section 4.18 (Admin Products):*
- `GET /admin/products`: L699 → L722
- `GET /admin/products/diagnostics`: L812 → L835
- `POST /admin/products/bulk-activate`: L893 → L916
- `PUT /admin/products/by-sku/:sku`: L962 → L985
- `GET /admin/products/:productId`: L1029 → L1052
- `PUT /admin/products/:productId`: L1128 → L1151
- `POST /admin/products/:productId/images`: L1255 → L1278
- `DELETE /admin/products/:productId/images/:imageId`: L1364 → L1387
- `PUT /admin/products/:productId/images/reorder`: L1445 → L1468

*Section 4.19 (Admin Business Applications):*
- `GET /admin/business-applications`: L2003 → L2035
- `GET /admin/business-applications/:applicationId`: L2076 → L2108
- `PUT /admin/business-applications/:applicationId`: L2149 → L2182
- `GET /admin/business-applications-stats`: L2250 → L2283

*Section 4.19b (Admin Customers):*
- `GET /admin/customers`: L1885 → L1917

*Section 4.19c (Admin Cron & Recovery):*
- Updated all 15 cron/recovery endpoint line numbers (23-164 line offsets throughout)

*Section 4.21 (Admin Analytics — New Dashboards):*
- Updated line numbers for all 38 endpoints in `adminAnalytics.js` (minor offsets of 1-3 lines)
- `GET /api/admin/analytics/overview`: L2004 → L2007
- `GET /api/admin/analytics/top-products`: L2105 → L2108
- `GET /api/admin/analytics/cac`: L787 → L788
- `GET /api/admin/analytics/ltv-cac-ratio`: L839 → L840
- `GET /api/admin/analytics/cohorts`: L888 → L889
- `GET /api/admin/analytics/churn`: L962 → L963
- `GET /api/admin/analytics/customer-health`: L1020 → L1021
- `GET /api/admin/analytics/nps`: L1114 → L1115
- `POST /api/admin/analytics/feedback`: L1154 → L1155
- `GET /api/admin/analytics/repeat-purchase`: L1192 → L1193
- `GET /api/admin/analytics/campaigns`: L1239 → L1240
- `POST /api/admin/analytics/campaigns`: L1300 → L1301
- `POST /api/admin/analytics/marketing-spend`: L1331 → L1332
- `GET /api/admin/analytics/channel-efficiency`: L1387 → L1388
- `GET /api/admin/analytics/conversion-funnel`: L1436 → L1437
- `GET /api/admin/analytics/inventory-turnover`: L1504 → L1505
- `GET /api/admin/analytics/dead-stock`: L1572 → L1573
- `GET /api/admin/analytics/stock-velocity`: L1632 → L1633
- `GET /api/admin/analytics/inventory-cash-lockup`: L1698 → L1700
- `GET /api/admin/analytics/product-performance`: L1752 → L1755
- `GET /api/admin/analytics/page-revenue`: L1831 → L1834
- `GET /api/admin/analytics/alerts`: L1894 → L1897
- `PUT /api/admin/analytics/alerts/:alertId/acknowledge`: L1927 → L1930
- `GET /api/admin/analytics/alert-thresholds`: L1953 → L1956
- `PUT /api/admin/analytics/alert-thresholds/:thresholdId`: L1972 → L1975
- GA4 summary: `admin.js:2297` → `admin.js:2340`

*Section 4.19c (Feed Files):*
- **Added Feed File Management** documentation: `POST /api/admin/feed-files/:feedType` (L2977) and `GET /api/admin/feed-files` (L3016)

*Section 4.16 (Webhooks):*
- Raw body middleware: `server.js:189` → `server.js:190`

*Section 4.23 (Utility Endpoints):*
- Health, ready, root line refs updated: L206→L207, L227→L228, L257→L258

*Section 8 (Security Model):*
- Helmet: `server.js:108-120` → `server.js:109-121`
- Global rate limit: `server.js:172-177` → `server.js:173-177`
- Contact rate limit: `contact.js:11-17` → `contact.js:12-18`
- HTTPS enforcement: `server.js:122-131` → `server.js:124-132`
- Body size limits: `server.js:192-193` → `server.js:193-194`
- Trust proxy: `server.js:102` → `server.js:103`
- Webhook raw body: `server.js:189` → `server.js:190`

*Section 12 (Changelog):*
- Marked ribbon `compatibility` field changelog entry as NOT IMPLEMENTED

**Frontend impact:** None. Documentation-only corrections. All API contracts, response shapes, and endpoint behaviors are unchanged. The removed Section 4.21 endpoints were never implemented — no working frontend code could have depended on them.

---

### 2026-03-04 — Remove Ribbon Import from Cron + Admin Endpoint

**Summary:** Removed ribbon import (`import-ribbons.js`) from the daily cron job and removed the `POST /api/admin/import/ribbons` admin endpoint. The daily import now only runs genuine and compatible scripts. Ribbon data is managed separately and is no longer auto-imported.

**Files changed:**
- `src/cron/jobs.js` — Removed `{ name: 'ribbons', file: 'import-ribbons.js' }` from `dailyProductImport()` scripts array. Updated log messages.
- `src/cron/scheduler.js` — Updated import failure alert text (2 scripts, not 3). Updated scheduler startup log.
- `src/routes/admin.js` — Removed `POST /api/admin/import/ribbons` endpoint (71 lines).

**Removed endpoints:**

| Method | Path | Status |
|--------|------|--------|
| POST | `/api/admin/import/ribbons` | **Removed** |

**Updated endpoints:**

| Method | Path | Change |
|--------|------|--------|
| POST | `/api/admin/cron/daily-product-import` | Now runs genuine → compatible only (no ribbons) |

**No frontend breaking changes.** All public ribbon endpoints (`/api/ribbons/*`) and admin ribbon CRUD (`/api/admin/ribbons/*`) continue to work unchanged. Only the automated import trigger was removed.

**Updated sections:** 4.19c (admin cron table — removed ribbon import row, updated daily-product-import description), 2026-03-02 changelog (cross-references updated).

---

### 2026-03-04 — Fix Sentry Instrumentation Order

**Summary:** Moved `Sentry.init()` before `require('express')` in `src/server.js` so Sentry can monkey-patch express for proper error tracking and tracing. Previously express was imported on line 2, but Sentry wasn't initialized until line 22 — causing the warning: *"express is not instrumented"*.

**Files changed:**
- `src/server.js` — Sentry require + `Sentry.init()` block moved from after all imports (L10, L21-27) to immediately after `dotenv` (L7-14), before express and other module imports. Net +1 line shift on all subsequent code.

**No frontend breaking changes.** No API changes — this is a server startup order fix only.

**Updated sections:** Middleware order line refs (Section 1), utility endpoints line refs (Section 4.23), security model line refs (Section 8 — Helmet, rate limit, body size, trust proxy, webhook raw body).

---

### 2026-03-05 — Fix Ribbon Compatibility Parsing + ODS Feed Integration

**Summary:** Fixed `ribbon_compatibility` parsing bugs that produced incorrect entries from the ODS "For use in:" column. Added 18 missing brand aliases, fixed multi-brand slash detection, repeated brand splitting, and space-separated model parsing. All three import scripts (`genuine.js`, `compatible.js`, `import-ribbons.js`) now produce correct compatibility entries via the shared `syncRibbonCompatibility()` pipeline. Entry count jumped from 204 (name-only parsing) to ~1,341 (1,257 ODS + 66 genuine + 18 compatible).

**Files changed:**
- `scripts/ribbon-compat/config.js` — Added 18 brand aliases (3M, Imation, Unisys, Facit, Sanyo, ABM, etc.). Refactored `parseCompatEntry()` with three new parsing strategies: (1) multi-brand slash detection for `3M/IMATION 200` and `Burroughs/Unisys C2000 / C2050` patterns, (2) `trySplitOnRepeatedBrands()` for `Sharp PA3000 Sharp PA3000H` patterns, (3) `trySplitSpaceSeparatedModels()` for `PA3300 PA3320 XQ315` patterns. Added brand carry-forward in `parseCompatibilityText()` for `Facit, 1610, 1620` comma-separated patterns. Extracted `extractBrandFromStart()` helper. Fixed `CONFLICT_SQL` to use `confidence` instead of removed `source_rank` column.
- `scripts/import-ribbons.js` — Added `syncRibbonCompatibility` call to populate `ribbon_compatibility` from ODS "For use in:" column during import. Builds `skuToId` map and passes compatibility text through shared parsing pipeline.
- `scripts/genuine.js` — Added `syncRibbonCompatibility` call (Step 14.3) for genuine ribbon products using `PRINTER_COMPAT` column data.
- `scripts/compatible.js` — Added `syncRibbonCompatibility` call (Step 13.7.5) for compatible ribbon products (name parsing only — Augmento feed has no compatibility column).
- `src/utils/feedHelpers.js` — Added `syncRibbonCompatibility()` function: deletes existing entries per-ribbon, parses both `compatible_printers` text (confidence 70) and product name (confidence 50), deduplicates via seen set, batch inserts. Imports `parseCompatibilityText`, `parseRibbonName`, `normalizeBrand`, `normalizeModel` from `scripts/ribbon-compat/config.js`.
- `scripts/ribbon-compat/phase-a-seed.js` — Removed (functionality moved to `syncRibbonCompatibility` in feedHelpers.js).
- `scripts/ribbon-compat/phase-b-enrich.js` — Updated to use products table.
- `scripts/ribbon-compat/report.js` — Removed unused `ribbonIds` variable.

**Ribbon compatibility entry counts by source:**

| Source | Ribbons | Compat Entries | Data Source |
|--------|---------|----------------|-------------|
| `ribbon` (ODS) | 95 | ~1,257 | "For use in:" column (highest coverage) |
| `genuine` (DSNZ) | 11 | ~66 | Column 34 `PRINTER_COMPAT` |
| `compatible` (Augmento) | 9 | ~18 | Product name parsing only (no compat column) |

**No frontend breaking changes.** The `ribbon_compatibility` table schema is unchanged — only data volume increases. All existing ribbon endpoints (`/api/ribbons`, `/api/ribbons/:sku`, device filtering RPC) benefit automatically from richer data.

**Updated sections:** This changelog entry (Section 10).

### 2026-03-08 — Security Audit: PostgREST Injection, Stored XSS, Import Hardening

**Summary:** Adversarial security audit identifying and patching 9 vulnerabilities across search endpoints, admin text fields, import scripts, order responses, and email handling. Also added role restriction to `GET /admin/products` and import deactivation preservation.

**Files changed:**
- `src/utils/response.js` — Added `sanitizeSearchForPostgREST()` helper that escapes LIKE wildcards AND strips PostgREST filter syntax characters (commas, parentheses) to prevent `.or()` filter injection.
- `src/routes/products.js` — Search uses `sanitizeSearchForPostgREST()` instead of inline LIKE-only escaping.
- `src/routes/ribbons.js` — Same search sanitization fix.
- `src/routes/admin.js` — (1) Search sanitization applied to all 4 `.or()` call sites (orders, products, customers, refunds). (2) Added `requireRole('super_admin', 'stock_manager')` to `GET /admin/products` — `order_manager` can no longer access product listings with `cost_price`. (3) Import error responses no longer return `error.message` or `stderr` — generic messages only. (4) Import success responses return parsed `summary` object instead of raw stdout. (5) Added `sanitize-html` stripping to: `admin_notes` on order updates, `order_events.payload.note`, `business_application.notes`, `refund.reason_note`, product `name`/`description`/`meta_title`/`meta_description`/`meta_keywords`.
- `src/routes/reviews.js` — `admin_notes` on review moderation now sanitized with `sanitize-html` (user-submitted review `title`/`body` were already sanitized).
- `src/routes/orders.js` — Removed internal DB error code (`DB: ${dbCode}`) from client-facing error response. Code logged server-side only.
- `src/routes/contact.js` — Strip `\r\n` from email subject to prevent header injection.
- `scripts/genuine.js` — Product names from DSNZ feed now HTML-stripped via `sanitize-html` during import.
- `scripts/compatible.js` — Product names from Augmento feed now HTML-stripped via `sanitize-html` during import.
- `src/utils/feedHelpers.js` — `batchUpsertWithRetry()` now preserves deactivated products: before each batch upsert, queries which SKUs are currently `is_active=false`, then restores their deactivated state after upsert. Prevents feed imports from silently reactivating manually deactivated products. Returns `preservedDeactivations` count in result.

**Frontend impact:**
- **`GET /api/admin/products`** — `order_manager` admins will now receive 403 (previously allowed). Only `super_admin` and `stock_manager` can access.
- **Import success responses** — `output` field replaced by `summary` (parsed JSON object or `null`). Update any admin UI that displays raw import output.
- **Import error responses** — No longer include `error` or `stderr` fields in error details. Only the top-level error message is returned.
- **Search endpoints** — Commas and parentheses in search queries are now stripped. No visible change for normal searches; prevents filter injection.
- **Admin text fields** — HTML tags are now stripped from all admin-submitted text. No change for normal text input; `<script>` or `<img>` tags will be removed.
- **Order creation errors** — Generic "Failed to create order" message (previously included `(DB: XXXXX)` suffix).

**Updated sections:** Section 4.2 (admin products auth), Section 10 (changelog).

---

### 2026-03-08 — Pack Image Generation + Series Key Fix

**Summary:** Color packs now get auto-generated composite images during import. The `extractSeriesKey` function was refactored to prioritize MPN over SKU for more accurate series grouping, which changes pack SKU naming for many products. A one-time cleanup script deactivates old packs before re-import.

**Files changed:**
- `src/utils/packImageGenerator.js` — **[NEW]** Composite image generator using `sharp`. Downloads constituent cartridge images, resizes to uniform height (400px), arranges side-by-side in canonical color order (KCMY or CMY), and outputs a PNG. Supports Playwright browser for Cloudflare-protected image URLs. Uploads to Supabase storage at `images/products/{packSKU}/composite.png` and creates `product_images` record.
- `src/utils/colorPackGenerator.js` — Added Step I (image generation) after pack upsert. Passes constituent product data through to `generatePackImages()`. Skips packs that already have an `image_url` or where any constituent lacks an image. Exports `BRAND_PREFIX` constant.
- `src/services/colorPackService.js` — Refactored `extractSeriesKey()` to prioritize MPN-based extraction over SKU-based. New priority order: (1) MPN with hyphens stripped + pack/color suffixes removed, (2) model number from product name, (3) SKU-based fallback, (4) generic alphanumeric prefix. This produces more accurate and stable series keys (e.g., MPN `LC-3337BK` → `LC3337` instead of SKU `B3337B` → `B3337`).
- `src/cron/jobs.js` — Updated `dailyProductImport` comments to reflect that ribbon import is no longer part of the cron pipeline.
- `package.json` — Added `sharp` (image processing) and `playwright` (dev dependency for Cloudflare bypass in backfill script).
- `scripts/cleanup-old-pack-skus.js` — **[NEW]** One-time script to deactivate all existing auto-generated packs before re-import with new MPN-based SKUs. Prevents 30% circuit breaker trigger.
- `scripts/generate-pack-images.js` — **[NEW]** Backfill script to generate composite images for existing packs. Uses Playwright for Cloudflare-protected supplier image URLs. Supports `--confirm`, `--force`, `--limit`, `--source` flags.

**Frontend impact:**
- **Color pack `image_url`** — Packs that previously had `null` for `image_url` will now have composite images (where constituent images exist). Frontend `<img>` rendering works unchanged — the URL is resolved server-side.
- **Pack SKUs may change** — Many pack SKUs will change from SKU-based to MPN-based naming (e.g., `GEN-PACK-B3337-CMY` → `GEN-PACK-LC3337-CMY`). If the frontend stores or caches pack SKUs, these will need refreshing. Product UUIDs remain stable via upsert-on-SKU.
- **No API contract changes** — All endpoints return the same shape. `image_url` field was already nullable; now more packs will have non-null values.

**Updated sections:** Section 1 (sharp dependency), Section 7.4 (color pack images note).

### 2026-03-08 — Line Reference Audit (server.js, index.js, products.js, admin.js, adminShipping.js, adminSupplierOffers.js)

**No code changes — documentation-only update.**

Fixed ~60 stale `file:line` references caused by commits since 2026-03-04 (ribbon margin reduction, pack image generation, series key fix, test cleanup script, circuit breaker fix).

*Key shifts:*
- `server.js`: Entry point L375→L376, HTTPS redirect L124→L125
- `routes/index.js`: All route mounts shifted +4 (L268-288 → L272-293). Added missing `adminSupplierOffers.js` at L292.
- `products.js`: 7 endpoints shifted -1 (L205→L204 through L627→L626)
- `admin.js`: 40+ endpoint refs shifted +2 to +13 depending on position (accumulated from multiple commits)
- `adminShipping.js`: GET rates L66→L119 (helper functions `detectGaps`/`findOverlappingRate` added before route handlers)
- `adminSupplierOffers.js`: L14→L13
- Feed-files: L2977→L2990, L3016→L3029
- Rate limiter: `admin.js:135-142` → `admin.js:137-144`

*Files verified as correct (no changes needed):*
orders.js, cart.js, search.js, auth.js, user.js, reviews.js, ribbons.js, adminRibbons.js, business.js, shipping.js, settings.js, contact.js, newsletter.js, account.js, cartAnalytics.js, webhooks.js, adminAnalytics.js, analytics.js, middleware/auth.js
