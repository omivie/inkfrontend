...
**Authenticated-only cart operations:**
```
POST /api/cart/merge      → Merge guest cart into user cart (call after login)
POST /api/cart/validate   → Validate cart (stock check, price refresh)
POST /api/cart/coupon      → Apply coupon     body: { coupon_code }  (requires verified email)
