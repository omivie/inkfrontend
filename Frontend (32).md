...
> **Updated:** 2026-03-02 (label_tape split from ribbon, consumable/cartridge filters exclude ribbons+label_tape, search ribbon/label_tape support, ribbon compatibility enrichment) | **Generated:** 2026-02-18 | **Source:** Zero-assumption audit of current backend source code
...
| POST | `/api/admin/import/genuine` | L2946 | Trigger genuine products import (DSNZ.xlsx). 10-min timeout. 409 if already running. |
| POST | `/api/admin/import/compatible` | L2996 | Trigger compatible products import (Augmento.xlsx). 10-min timeout. 409 if already running. |
...
| `ribbons` | **LEGACY** — fully superseded by `products` table with `product_type='ribbon'`. All three ribbon sources (genuine, compatible, ODS) now write to `products`. Table still exists but is not written to by any import script |
...
| `022_ribbons.sql` | Original `ribbons` table with RLS and indexes (ribbons now unified into `products` table with `product_type='ribbon'`) |
